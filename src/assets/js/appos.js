...
ace.general_things = function($) {
 $('.appos-nav [class*="icon-animated-"]').closest('a').on('click', function(){
 ...
 $('#appos-settings-btn').on(ace.click_event, function(){
	$(this).toggleClass('open');
	$('#appos-settings-box').toggleClass('open');
 });

 
 $('#appos-settings-navbar').on('click', function(){
	ace.settings.navbar_fixed(this.checked);//@ ace-extra.js
 }).each(function(){this.checked = ace.settings.is('navbar', 'fixed')})

 $('#appos-settings-sidebar').on('click', function(){
 ...
 $('#btn-scroll-up').on(ace.click_event, function(){
  ...
  $('#skin-colorpicker').on('change', function(){
	...
	if(skin_class == 'skin-1') {
		$('.appos-nav > li.grey').addClass('dark');
	}
	else {
		$('.appos-nav > li.grey').removeClass('dark');
	}

	if(skin_class == 'skin-2') {
		$('.appos-nav > li').addClass('no-border margin-1');
		$('.appos-nav > li:not(:last-child)').addClass('light-pink').find('> a > [class*="icon-"]').addClass('pink').end().eq(0).find('.badge').addClass('badge-warning');
	}
	else {
		$('.appos-nav > li').removeClass('no-border margin-1');
		$('.appos-nav > li:not(:last-child)').removeClass('light-pink').find('> a > [class*="icon-"]').removeClass('pink').end().eq(0).find('.badge').removeClass('badge-warning');
	}

	if(skin_class == 'skin-3') {
		$('.appos-nav > li.grey').addClass('red').find('.badge').addClass('badge-yellow');
	} else {
		$('.appos-nav > li.grey').removeClass('red').find('.badge').removeClass('badge-yellow');
	}
 });
